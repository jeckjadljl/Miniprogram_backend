...
const { transaction } = require("../extend/application.js");

module.exports = app => {
  const { Sequelize, model, getSortInfo, checkUpdate, checkDelete } = app;
  const { Op } = Sequelize;
  const freightPlanSchema = require("../schema/freight_plan.js")(app);
  const FreightPlan = model.define("freight_plan", freightPlanSchema);
